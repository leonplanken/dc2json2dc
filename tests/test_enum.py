...
d4_float_json = (
    '{"name": "name1", "my_enum": 1.23, "enum_list": [], "nested": {}}'
)

d5_list = DataWithEnum(
    "name1",
    MyEnum.STR1,
    [MyEnum.STR2, MyEnum.STR3],
    nested={"enum_val": [MyEnum.STR1]},
)
...
flag_container = FlagContainer(flag_list=[MyFlag.A, MyFlag.C], flag=MyFlag.A)


class TestEncoder:
    @pytest.mark.skip
    def test_data_with_enum(self):
        assert d1.to_json() == d1_json, (
            f"Actual: {d1.to_json()}, Expected: {d1_json}"
        )
        assert d3_int.to_json() == d3_int_json, (
            f"Actual: {d3_int.to_json()}, Expected: {d3_int_json}"
        )
        assert d4_float.to_json() == d4_float_json, (
            f"Actual: {d4_float.to_json()}, Expected: {d4_float_json}"
        )

    def test_data_with_str_enum(self):
        assert ds.to_json() == ds_json, (
            f"Actual: {ds.to_json()}, Expected: {ds_json}"
        )
    ...
    def test_data_with_enum_default_value(self):
        ...
        assert d2_to_json == d2_json, (
            f"A default value was not included in the JSON encode. "
    ...
    def test_enum_with_list(self):
        assert d5_list.to_json() == d5_list_json, (
            f"Actual: {d5_list.to_json()}, Expected: {d5_list_json}"
        )
        assert d5_list.to_dict(encode_json=True) == json.loads(d5_list_json), (
            f"Actual: {d5_list.to_dict()}, Expected: {json.loads(d5_list_json)}"
        )

    def test_enum_flag(self):
        assert flag_container.to_json() == flag_container_json, (
            f"Actual: {flag_container.to_json()}, Expected: {flag_container_json}"
        )
        assert flag_container.to_dict(encode_json=True) == json.loads(
            flag_container_json
        ), (
            f"Actual: {flag_container.to_dict()}, Expected: {json.loads(flag_container_json)}"
        )
