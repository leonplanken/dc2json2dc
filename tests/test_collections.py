...
from tests.entities import (
    ...
    DataClassWithAbstractSet,
    DataClassWithCollection,
    ...
    DataClassWithMutableMapping,
    DataClassWithMutableSequence,
    DataClassWithMutableSet,
    DataClassWithMyCollection,
    DataClassWithOptional,
    DataClassWithOptionalStr,
    DataClassWithOptionalUnbound,
    DataClassWithSequence,
    DataClassWithSet,
...
class TestEncoder:
    ...
    def test_dict_unbound(self):
        assert (
            DataClassWithDictUnbound({"1": "a"}).to_json()
            == '{"kvs": {"1": "a"}}'
        )

    def test_dict_builtin(self):
        assert (
            DataClassWithDictBuiltin({"1": "a"}).to_json()
            == '{"kvs": {"1": "a"}}'
        )

    def test_dict_int(self):
        assert (
            DataClassWithDictInt({1: "a"}).to_json() == '{"kvs": {"1": "a"}}'
        )
    ...
    def test_frozenset(self):
        assert (
            DataClassWithFrozenSet(frozenset([1])).to_json() == '{"xs": [1]}'
        )

    def test_frozenset_unbound(self):
        assert (
            DataClassWithFrozenSetUnbound(frozenset([1])).to_json()
            == '{"xs": [1]}'
        )

    def test_frozenset_builtin(self):
        assert (
            DataClassWithFrozenSetBuiltin(frozenset([1])).to_json()
            == '{"xs": [1]}'
        )
    ...
    def test_deque_builtin(self):
        assert (
            DataClassWithDequeCollections(deque([1])).to_json()
            == '{"xs": [1]}'
        )
    ...
    def test_my_collection(self):
        assert (
            DataClassWithMyCollection(MyCollection([1])).to_json()
            == '{"xs": [1]}'
        )
    ...
    def test_counter(self):
        assert (
            DataClassWithCounter(c=Counter("foo")).to_json()
            == '{"c": {"f": 1, "o": 2}}'
        )
...
class TestDecoder:
    def test_list(self):
        assert DataClassWithList.from_json('{"xs": [1]}') == DataClassWithList(
            [1]
        )

    def test_list_unbound(self):
        assert DataClassWithListUnbound.from_json(
            '{"xs": [1]}'
        ) == DataClassWithListUnbound([1])

    def test_list_builtin(self):
        assert DataClassWithListBuiltin.from_json(
            '{"xs": [1]}'
        ) == DataClassWithListBuiltin([1])

    def test_list_str(self):
        assert DataClassWithListStr.from_json(
            '{"xs": ["1"]}'
        ) == DataClassWithListStr(["1"])

    def test_dict(self):
        assert DataClassWithDict.from_json(
            '{"kvs": {"1": "a"}}'
        ) == DataClassWithDict({"1": "a"})

    def test_dict_unbound(self):
        assert DataClassWithDictUnbound.from_json(
            '{"kvs": {"1": "a"}}'
        ) == DataClassWithDictUnbound({"1": "a"})

    def test_dict_builtin(self):
        assert DataClassWithDictBuiltin.from_json(
            '{"kvs": {"1": "a"}}'
        ) == DataClassWithDictBuiltin({"1": "a"})

    def test_dict_int(self):
        assert DataClassWithDictInt.from_json(
            '{"kvs": {"1": "a"}}'
        ) == DataClassWithDictInt({1: "a"})

    def test_set(self):
        assert DataClassWithSet.from_json('{"xs": [1]}') == DataClassWithSet(
            {1}
        )

    def test_set_unbound(self):
        assert DataClassWithSetUnbound.from_json(
            '{"xs": [1]}'
        ) == DataClassWithSetUnbound({1})

    def test_set_builtin(self):
        assert DataClassWithSetBuiltin.from_json(
            '{"xs": [1]}'
        ) == DataClassWithSetBuiltin({1})

    def test_tuple(self):
        assert DataClassWithTuple.from_json(
            '{"xs": [1]}'
        ) == DataClassWithTuple((1,))

    def test_tuple_unbound(self):
        assert DataClassWithTupleUnbound.from_json(
            '{"xs": [1]}'
        ) == DataClassWithTupleUnbound((1,))

    def test_tuple_builtin(self):
        assert DataClassWithTupleBuiltin.from_json(
            '{"xs": [1]}'
        ) == DataClassWithTupleBuiltin((1,))

    def test_frozenset(self):
        assert DataClassWithFrozenSet.from_json(
            '{"xs": [1]}'
        ) == DataClassWithFrozenSet(frozenset([1]))

    def test_frozenset_unbound(self):
        assert DataClassWithFrozenSetUnbound.from_json(
            '{"xs": [1]}'
        ) == DataClassWithFrozenSetUnbound(frozenset([1]))

    def test_frozenset_builtin(self):
        assert DataClassWithFrozenSetBuiltin.from_json(
            '{"xs": [1]}'
        ) == DataClassWithFrozenSetBuiltin(frozenset([1]))

    def test_deque(self):
        assert DataClassWithDeque.from_json(
            '{"xs": [1]}'
        ) == DataClassWithDeque(deque([1]))

    def test_deque_unbound(self):
        assert DataClassWithDequeUnbound.from_json(
            '{"xs": [1]}'
        ) == DataClassWithDequeUnbound(deque([1]))

    def test_deque_collections(self):
        assert DataClassWithDequeCollections.from_json(
            '{"xs": [1]}'
    ...
    def test_optional_unbound(self):
        assert DataClassWithOptionalUnbound.from_json(
            '{"x": 1}'
    ...
    def test_optional_str(self):
        assert DataClassWithOptionalStr.from_json(
            '{"x": "1"}'
        ...
        assert (
            DataClassWithOptionalStr.from_json("{}", infer_missing=True)
            == DataClassWithOptionalStr()
        )

    def test_my_collection(self):
        assert DataClassWithMyCollection.from_json(
            '{"xs": [1]}'
        ) == DataClassWithMyCollection(MyCollection([1]))

    def test_immutable_default(self):
        assert (
            DataClassIntImmutableDefault.from_json('{"x": 0}')
            == DataClassIntImmutableDefault()
        )
        assert (
            DataClassMutableDefaultList.from_json("{}", infer_missing=True)
            == DataClassMutableDefaultList()
        )

    def test_mutable_default_list(self):
        assert (
            DataClassMutableDefaultList.from_json('{"xs": []}')
            == DataClassMutableDefaultList()
        )
        assert (
            DataClassMutableDefaultList.from_json("{}", infer_missing=True)
    ...
    def test_mutable_default_dict(self):
        assert (
            DataClassMutableDefaultDict.from_json('{"kvs": {}}')
            == DataClassMutableDefaultDict()
        )
        assert (
            DataClassMutableDefaultDict.from_json("{}", infer_missing=True)
            == DataClassMutableDefaultDict()
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                '{"c": [1, 2]}',
                DataClassWithCollection((1, 2)),
                id="collection",
            ),
            pytest.param(
                '{"c": [1, 2]}', DataClassWithSequence((1, 2)), id="sequence"
            ),
            pytest.param(
                '{"c": [1, 2]}',
                DataClassWithMutableSequence([1, 2]),
                id="mutable-sequence",
            ),
            pytest.param(
                '{"c": [1, 2]}', DataClassWithAbstractSet({1, 2}), id="set"
            ),
            pytest.param(
                '{"c": [1, 2]}',
                DataClassWithMutableSet({1, 2}),
                id="mutable-set",
            ),
            pytest.param(
                '{"c": {"1": 1, "2": 2}}',
                DataClassWithMapping({"1": 1, "2": 2}),
                id="mapping",
            ),
            pytest.param(
                '{"c": {"1": 1, "2": 2}}',
                DataClassWithMutableMapping({"1": 1, "2": 2}),
                id="mutable-mapping",
            ),
        ],
    )
    def test_abstract_collections(self, json_string, expected_instance):
        assert (
            type(expected_instance).from_json(json_string) == expected_instance
        )
